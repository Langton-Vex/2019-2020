...
#define WHEEL_CIRCUMFERENCE 4.125 * IN_TO_CM * PI / 100

Chassis::Chassis(){

	float turn_radius = TURN_RADIUS; // made a macro for convenience,
	float wheel_circumference = WHEEL_CIRCUMFERENCE;// But OO is good practice
...
void Chassis::move_forward(float distance,int velocity=100){
	float rel_target = distance / wheel_circumference;
	float abs_target = peripherals.left_mtr.get_position() + rel_target;
...
void Chassis::point_turn(int angle,int velocity=100){
	float rel_target = (2*turn_radius*PI * (angle/360)) / wheel_circumference;
	float abs_target = peripherals.left_mtr.get_position() + rel_target;
